...
class FurutaPIDFrame(wx.Frame):
    ...
    def _fake_encoder_thread(self):
        ...
        L1      = ARM_LENGTH_M
        L2      = PEND_LENGTH_M
        damping = 0.3           # light pivot friction
        dt      = 1.0 / SYNC_HZ

        # EMA coefficients — 5 Hz cutoff on both arm velocity and acceleration.
        # Low cutoff keeps the double-derivative from amplifying quantisation noise.
        a = min(1.0, 2 * math.pi * 5.0 / SYNC_HZ)

        theta     = math.pi   # hanging straight down
        theta_dot = 0.0

        with self._lock:
            prev_arm_cts = self._puck1_pos
        arm_vel_filt = 0.0
        arm_acc_filt = 0.0

        # Hold at rest until motor enables so auto-zero sees a stable window.
        while self._connected and self._fake_enc and not self._enabled:
            with self._lock:
                prev_arm_cts = self._puck1_pos   # stay current while waiting
            time.sleep(0.02)
        if not (self._connected and self._fake_enc):
            return

        # Small initial kick to break the symmetry at θ = π so the energy
        # controller's vel_slow threshold (0.1 rad/s) is crossed quickly.
        theta_dot = 0.5
        ...
        while self._connected and self._fake_enc:
            ...
            with self._lock:
                arm_cts = self._puck1_pos
            d_cts        = arm_cts - prev_arm_cts
            prev_arm_cts = arm_cts

            # Arm angular velocity from encoder counts, then EMA-smoothed.
            arm_vel_raw  = d_cts * 2.0 * math.pi / ENCODER_RES / act_dt
            arm_vel_new  = a * arm_vel_raw  + (1.0 - a) * arm_vel_filt

            # Arm angular acceleration from velocity derivative, then EMA-smoothed
            # and clamped to prevent quantisation spikes from destabilising the ODE.
            arm_acc_raw  = (arm_vel_new - arm_vel_filt) / act_dt
            arm_acc_filt = max(-80.0, min(80.0,
            ...
            # Control loop reads: pend_rad = _wrap(p2 * 2π/ENC + π)
            # so p2 = (θ − π) * ENC / (2π)  gives pend_rad = θ (before wrap).
            with self._lock:
                self._puck2_pos = int(
