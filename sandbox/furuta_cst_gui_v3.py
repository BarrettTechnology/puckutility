...
KP_DEFAULT          = 500    # [torque / rad]
KI_DEFAULT          = 0      # [torque / (rad·s)]
KD_DEFAULT          = 30     # [torque / (rad/s)]
ADZ_DEFAULT         = 1      # [deg]
BI_DEFAULT          = 0      # [deg]
...
PDZ_DEFAULT         = 150    # [deg]
TMAX_DEFAULT        = 4      # [deg]
KS_DEFAULT          = 800    # [torque units] peak swingup torque
KB_DEFAULT          = 800    # [torque units] peak braking torque
KV_DEFAULT          = 8000   # [torque units/s] torque slew-rate limit
KDA_DEFAULT         = 200    # [torque units / (rad/s)] arm velocity damping during swingup
TORQUE_LIMIT_DEFAULT = 2000  # [torque units] hard output clamp (replaces error limit)
DISABLE_TEMP        = 85
...
class FurutaPIDFrame(wx.Frame):
    ...
    def _build_ui(self):
        ...
        self._torque_lim = gain(row3, "Torque limit:", TORQUE_LIMIT_DEFAULT,
            "Hard output clamp on torque command [drive units]. Lower to protect against overheating.")
    ...
    def _start_control(self):
        try:
            ...
            torque_lim = float(self._torque_lim.GetValue())
        except ValueError:
        ...
        for tc in (self._kp, self._ki, self._kd, self._adz,
                   ...
                   self._ks, self._kb, self._kv, self._kda, self._torque_lim):
            tc.Disable()
        if self._no_swing:
        ...
        self._ctrl_thread = threading.Thread(
            ...
            args=(kp, ki, kd, adz, bi, kt, kf, pdz, tmax, ks, kb, kv, kda, torque_lim),
            daemon=True)
    ...
    def _control_loop(self, kp, ki, kd, adz, bi, kt, kf, pdz, tmax, ks, kb, kv, kda, torque_lim):
        # ── Parameter reference ───────────────────────────────────────────
        #
        ...
        # Clamp torque limit to a positive value
        torque_lim = max(0.0, torque_lim)
        ...
        while self._controlling and self._enabled:
            ...
            if in_balance:
                ...
                torque   = balance_ramp * (kp * pend_rad_dz
                                           + ki * integral
                                           + kd * vel_fast)
                torque   = max(-torque_lim, min(torque_lim, torque))
                ...
                torque       = max(-torque_lim, min(torque_lim, -kda * arm_vel))
                self._in_braking = False
